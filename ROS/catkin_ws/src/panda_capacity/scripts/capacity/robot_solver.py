...
class RobotSolver:
    ...
    def force_polytope_intersection(self, q1,q2):
        ...
        vertex, faces_index =  capacity_solver.force_polytope_intersection_withfaces(Jac1,Jac2,self.t_max,self.t_min,self.t_max,self.t_min, self.gravity_torque(q1), self.gravity_torque(q2))
    ...
    def force_polytope_sum(self, q1,q2):
        ...
        Jac1 = self.jacobian_position(q1)
        Jac2 = self.jacobian_position(q2)
        vertex, faces_index =  capacity_solver.force_polytope_sum_withfaces(Jac1,Jac2,self.t_max,self.t_min,self.t_max,self.t_min, self.gravity_torque(q1), self.gravity_torque(q2))
        faces = capacity_solver.face_index_to_vertex(vertex,faces_index)
        return vertex, faces
    ...
    def force_polytope(self, q, direction = None, force = None):
        ...
        if direction is not None:
            ...
            Jac = Jac_full
        vertex, faces_index =  capacity_solver.force_polytope_withfaces(Jac, self.t_max, self.t_min, self.gravity_torque(q))
        faces = capacity_solver.face_index_to_vertex(vertex,faces_index)
        return vertex, faces
